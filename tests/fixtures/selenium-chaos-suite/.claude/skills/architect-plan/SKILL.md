...
- src/core/LegacyActions.ts (121 LOC)
- tests/mega-suite.spec.ts (79 LOC)
- tests/regression.spec.ts (73 LOC)
- src/pages/BasePage.ts (66 LOC)
- src/pages/ProductsPage.ts (66 LOC)

**Hub files** (imported by many others  -  high-impact, higher risk to touch):
- src/core/DriverFactory.ts (13 dependents)
- src/data/users.ts (13 dependents)
- src/pages/LoginPage.ts (13 dependents)
- src/core/WaitUtils.ts (10 dependents)
- src/pages/ProductsPage.ts (10 dependents)

**Code duplication**: 4.7% of lines are duplicated
...
- tests/e2e: E2E test specs organized by user flow or feature. Each spec file tests one major user journey. Specs import Page Objects from tests/e2e/pages/ and the driver factory from tests/e2e/fixtures/  -  they never contain raw driver.findElement() calls or By selectors.

**How data should flow through this architecture**:
Test → POM Methods (explicit waits) → WebDriver → Browser → Application
...
Based on what you read, confirm or correct the detected stack (`Selenium E2E`). If the stack
looks wrong, call:
```
...
Otherwise call:
```
architect context --techstack selenium-e2e vitest-testing general-js
```

If `architect` is not found globally, fall back to:
```
npx @levironexe/architect context --techstack selenium-e2e vitest-testing general-js
```

This may return blueprints for several detected stacks. Apply the rules from each  -  the primary
stack (Selenium E2E) takes precedence on conflicts.
...
follows the Selenium E2E blueprint."
