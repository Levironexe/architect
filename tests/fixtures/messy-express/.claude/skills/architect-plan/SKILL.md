...
**Stack**: Express.js REST API

**Largest files by line count** (likely candidates for extraction):
- server.ts (301 LOC)
- src/config.ts (259 LOC)
- src/services/health.service.ts (21 LOC)
- src/services/route.service.ts (16 LOC)
- src/controllers/health.controller.ts (6 LOC)

**Hub files** (imported by many others  -  high-impact, higher risk to touch):
- src/config.ts (2 dependents)

**Code duplication**: 48.8% of lines are duplicated
...
- src/routes: Route definitions organized by resource.
- src/controllers: Request handlers that receive HTTP input, call services, and return responses.
- src/services: Business logic with no HTTP request or response awareness.
- src/models: Data models and database interactions.
- src/middleware: Cross-cutting request behavior such as auth, validation, and error handling.
...
Data flow: Route -> Controller -> Service -> Model
...
Based on what you read, confirm or correct the detected stack (`Express.js REST API`). If the stack
looks wrong, call:
```
...
Otherwise call:
```
architect context --techstack express-api general-js
```

If `architect` is not found globally, fall back to:
```
npx @levironexe/architect context --techstack express-api general-js
```

This may return blueprints for several detected stacks. Apply the rules from each  -  the primary
stack (Express.js REST API) takes precedence on conflicts.
...
follows the Express.js REST API blueprint."
