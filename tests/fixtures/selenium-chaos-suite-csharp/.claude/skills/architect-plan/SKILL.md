...
**Stack**: Selenium C# E2E

**Largest files by line count** (likely candidates for extraction):
- src/Pages/ProductsPage.cs (118 LOC)
- tests/MegaSuiteTests.cs (108 LOC)
- tests/RegressionTests.cs (106 LOC)
- src/Pages/BasePage.cs (82 LOC)
- tests/SmokeTests.cs (69 LOC)

**Hub files** (imported by many others  -  high-impact, higher risk to touch):


**Code duplication**: 9.6% of lines are duplicated
...
- Tests: Test specs organized by feature or user flow. Each test class tests one major user journey. Tests import Page Objects from Pages/ and the driver factory from Fixtures/  -  they never contain raw FindElement() calls or By selectors.

**How data should flow through this architecture**:
Test → Page Object → WebDriver (tests never touch WebDriver directly)
...
Based on what you read, confirm or correct the detected stack (`Selenium C# E2E`). If the stack
looks wrong, call:
```
...
Otherwise call:
```
architect context --techstack selenium-csharp
```

If `architect` is not found globally, fall back to:
```
npx @levironexe/architect context --techstack selenium-csharp
```

This may return blueprints for several detected stacks. Apply the rules from each  -  the primary
stack (Selenium C# E2E) takes precedence on conflicts.
...
follows the Selenium C# E2E blueprint."
